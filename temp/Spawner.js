...
Spawner.prototype.renewNearbyCreeps = function() {
  if(this.spawner !== undefined && !this.spawner.spawning) {
    var creeps = this.spawner.pos.findInRange(FIND_MY_CREEPS, 1).filter(function(creep) { return !_.includes(creep.body, MOVE) && creep.hits < config.renew_to_ttl }).sort(function(a, b) { return a.hits > config.critical_ttl ? 1 : -1 });
    if(creeps.length > 0) {
