...
Creep.prototype.renew = function() {
  ...
  if(this.pos.isNearTo(spawn)) {
    ...
    if(this.ticksToLive > config.critical_ttl && this.memory.role !== 'carrier' && (!this.originRoom().hasSpareEnergy() || this.ticksToLive > config.renew_to_ttl || Math.random() < config.stop_renew_prob)) {
